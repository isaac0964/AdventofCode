...
long extractNumber(std::string string) {
    // Extract number by concatenation of all numbers in string
    std::string number = "";
    std::regex pattern ("\\d+");
...
std::pair<int, int> getMaxMinTimesPressed(long t, long d){
...
int main() {
    ...
    long t = extractNumber(line);
    
    // Get race distane
    std::getline(file, line);
    long d = extractNumber(line);
    

    // Get min and max tp for given race
    std::pair<int, int>tps = getMaxMinTimesPressed(t, d);
    // The number of ways of breaking record is (tp_max-tp_min)

    std::cout << "The total numbers of ways you can beat the record is: " << (tps.second - tps.first) << std::endl;
